...
setwd('/home/larry/Github-Public/538-NFL-Predictions-2017/raw-data/oddsshark-lines/')
...
for (f in list.files()) {
  ...
  df <- data.frame(date.line = ymd_hm(str_sub(f, end = -16)), date.game = dates[1], team = teams)
  df <- cbind(df, lines)
